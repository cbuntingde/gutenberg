...
export const savePost =
	...
	async ( { select, dispatch, registry } ) => {
		...
		// Validate content before save to detect and warn about potential content issues.
		// This helps prevent silent save failures where content appears saved but is lost.
		const contentBeforeSave = content;
		...
		// FIX 4.2: Force clear dirty state after successful save to prevent
		// false "dirty" state that keeps save button stuck (Spectra/UAGB issue).
		// This ensures the save button properly returns to "Saved" state.
		...
		// Post-save validation: Detect silent save failures where content appears
		// to save but gets lost. This addresses the critical issue where "Post updated"
		// shows but content vanishes.
		if ( ! error && savedRecord && contentBeforeSave ) {
			try {
				const savedContent = savedRecord.content?.raw ?? savedRecord.content;
				// Check if saved content is significantly different from what was saved,
				// which could indicate a silent failure.
				const beforeLength = contentBeforeSave.trim().length;
				const afterLength = savedContent?.trim?.()?.length ?? 0;

				// Detect complete content loss (empty saved content)
				if (
					savedContent &&
					...
					// Silent save failure detected - saved content is empty but we had content.
					// This is a critical data integrity warning.
					registry
						.dispatch( noticesStore )
						.createWarningNotice(
							__(
								'Warning: The saved content appears to be empty. Please verify your content was saved correctly.'
							),
							{
								id: 'editor-content-warning',
								spoken: true,
							}
						);
				}
				// Detect partial content loss (significant reduction in content size)
				else if (
					savedContent &&
					typeof savedContent === 'string' &&
					beforeLength > 100 &&
					afterLength > 0 &&
					afterLength < beforeLength * 0.5
				) {
					// Significant content loss detected - more than 50% of content was lost.
					registry
						.dispatch( noticesStore )
						.createWarningNotice(
							__(
								'Warning: Only %s of content was saved. Please verify your content was saved correctly.',
								Math.round( ( afterLength / beforeLength ) * 100 )
							),
							{
								id: 'editor-content-warning',
								spoken: true,
				...
				// Silently ignore validation errors - don't block the save flow.
			}
		}
