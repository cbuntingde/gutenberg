...
/**
 * FIX 4.1: Large taxonomy optimization threshold
 * Switch from checkbox to search input when taxonomy has more than this many terms
 * to prevent performance issues with Toolset Types and similar plugins.
 * @default 100
 */
const LARGE_TAXONOMY_THRESHOLD = 100;
const EMPTY_ARRAY = [];
...
export function HierarchicalTermSelector( { slug } ) {
	...
	/**
	 * FIX 4.1: Large taxonomy optimization
	 * For taxonomies with many terms (exceeds LARGE_TAXONOMY_THRESHOLD),
	 * switch from checkbox list to search-based input to improve performance.
	 * This fixes Toolset Types performance issues.
	 */
	const isLargeTaxonomy = availableTerms.length > LARGE_TAXONOMY_THRESHOLD;
	...
	return (
		<Flex direction="column" gap="4">
			...
			{/* FIX 4.1: Show warning for large taxonomies */}
			{ isLargeTaxonomy && ! loading && (
				<div
					className="editor-post-taxonomies__large-taxonomy-notice"
					role="alert"
					style={ {
						padding: '8px 12px',
				...
				>
					{ __(
						'This taxonomy has many terms. Use search to filter below.',
						'gutenberg'
					) }
				</div>
			) }
